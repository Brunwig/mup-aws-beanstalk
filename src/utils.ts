...
async function retrieveEnvironmentInfo(api: MupApi, count: number, requestTime = new Date(Date.now() - 60 * 60 * 1000), infoType = 'tail'): Promise<EnvironmentInfoDescription[]> {
  ...
  if (EnvironmentInfo && EnvironmentInfo.length > 0) {
    ...
    const newestEntries = EnvironmentInfo.filter((log: any) => {
...
export async function downloadFullServerLogs(api: MupApi) {
  const config = api.getConfig();
  const { environment } = names(config);
  ...
  const filePaths = Promise.allSettled(latestData.map(async (bundleInfo: EnvironmentInfoDescription) => {
...
export async function selectPlatformArn(awsPlatformBranchName: string) {
